...
CLPAsymBlue::CLPAsymBlue() 
   : m_ignoreLoadWarnings (false), m_upperLowerErrorsOnly (true),
     m_lowerError (-1.), m_upperError (-1.)
{
                                    ...
                                    bool ignoreWarnings)
{
   if (ignoreWarnings)
   {
      if (mInd1 >= m_numMeasurements ||
          eInd1 >= m_numErrors)
      {
         // Don't pass go, don't bitch, just do nothing
         return;
      }
   } // if ignoreWarnings
   assert (mInd1 < m_numMeasurements);
...
void
CLPAsymBlue::getUpperErrorFraction (unsigned int mInd1, unsigned int eInd1, 
                                    DVec &value) const
{
   assert (mInd1 < m_numMeasurements);
   assert (eInd1 < m_numErrors);
   m_upperBlue.getErrorFraction (mInd1, eInd1, value);
}

void
CLPAsymBlue::getLowerErrorFraction (unsigned int mInd1, unsigned int eInd1, 
                                    DVec &value) const
{
   assert (mInd1 < m_numMeasurements);
   assert (eInd1 < m_numErrors);
   m_lowerBlue.getErrorFraction (mInd1, eInd1, value);
}
...
double 
CLPAsymBlue::lowerError (unsigned int mInd1, unsigned int eInd1, 
                         double mean)
{
   assert (mInd1 < m_numMeasurements);
   assert (eInd1 < m_numErrors);
   return m_lowerBlue.error (mInd1, eInd1, mean);
}

double 
CLPAsymBlue::upperError (unsigned int mInd1, unsigned int eInd1, 
                         double mean)
{
   assert (mInd1 < m_numMeasurements);
   assert (eInd1 < m_numErrors);
   return m_upperBlue.error (mInd1, eInd1, mean);
}
...
CLPAsymBlue::readFromFile(const string &filename)
{
   ...
   while (getline (source, line))
   {
      ...
      if ("lowerErrorVec" == sList[0])
      {
         ...
         setLowerErrorFraction (mInd1, eInd1, value, m_ignoreLoadWarnings);
         continue;
      }
      if ("upperErrorVec" == sList[0])
      {
         ...
         setUpperErrorFraction (mInd1, eInd1, value, m_ignoreLoadWarnings);
         continue;
      }
      if (size < 5)
      {
      ...
      if ("correlation" == sList[0])
      {
         ...
         setCorrelation (mInd1, mInd2, eInd1, value, m_ignoreLoadWarnings);
         continue;
      }
      // Should complain if we're here...
   } // while getline
   calcTotalErrors();
}
                                  ...
                                  double &lowerError, double errMean)
{
   ...
   m_upperError = upperError = factor * almostUpperError;
   m_lowerError = lowerError = factor * almostLowerError;
}
                                       ...
                                       double &lowerError)
{
   ...
   if (m_upperLowerErrorsOnly)
   {
      // Don't iterate - use the mean from the central BLUE
      // get lower error from lower blue
      m_lowerBlue.calcBlueAverage (dummy, almostLowerError, mean);
      // get upper error from upper blue
      m_centrBlue.calcBlueAverage (dummy, almostUpperError, mean);
   } else {
      // get lower error from lower blue
...
void
CLPAsymBlue::setMinimumMeanForErrors (double minMean)
{
   m_centrBlue.setMinimumMeanForErrors (minMean);
   m_lowerBlue.setMinimumMeanForErrors (minMean); 
   m_upperBlue.setMinimumMeanForErrors (minMean);
}
...
ostream& operator<< (ostream& o_stream, const CLPAsymBlue &rhs)
{
   if (rhs.aveError() > 0)
   {
      o_stream << "Combination:  " << rhs.fitMean()
               << " +" << rhs.upperError() << " -" << rhs.lowerError()
               << endl << endl;
   }
   for (CLPBlue::UInt measInd = 0; measInd < rhs.m_numMeasurements; ++measInd)
   {
      o_stream << "  Name   : " << rhs.name (measInd) << endl
               << "  mean   : " << rhs.indMean (measInd);
      if (rhs.aveError() > 0)
      {
         o_stream << endl << "  weight : " << rhs.weight (measInd);
      }
      o_stream << endl << endl;
   } // for measInd
