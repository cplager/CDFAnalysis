...
class CLPTemplateFitter
{
   public:
      ...
      CLPTemplateFitter &CLPTemplateFitter::operator= 
      (const CLPTemplateFitter& rhs);
      ...
      // off of sqrt(N)/N in each data bin.  Bins with 0 data events
      // are ignored.  User is responsible for the memory
      ...
      double getTotalBinContent (int binIndex) const;
      double getTotalBinContent (int binIndex, const DVec &paramVec) const;

      // returns the value of a parameter after the fit
      ...
      // that are not fixed.
      TMatrix getCovarianceMatrix (SVec &paramNamesVec, 
  ...
  private:
      ...
      // bin of a template.
      double _getBinFactor (int tempIndex, int binIndex,
                            double *arrayAddress = 0) const;
      double _getBinFactor (int tempIndex, int binIndex,
                            const DVec &paramVec) const;
      
      // returns bin contents for a template
      double _getBinContent (int tempIndex, int binIndex,
                             double *arrayAddress = 0) const;
      double _getBinContent (int tempIndex, int binIndex,
                             const DVec &paramVec) const;
      
      // returns the total bin contents for all templates
      double _getTotalBinContent (int binIndex,
                                  double *arrayAddress = 0) const;
      double _getTotalBinContent (int binIndex,
                                  const DVec &paramVec) const;

      // returns multiplicative factor by bin
      double _byBin (int tempIndex, int binIndex,
                     double *arrayAddress = 0) const;
      double _byBin (int tempIndex, int binIndex,
                     const DVec &paramVec) const;
      ...
      // returns true if fitter has parameter being fixed.
      bool _paramIsFixed (int paramIndex) const;
