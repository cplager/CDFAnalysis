...
$bin_dir = "/home/cplager/bin/print";
...
foreach $file (@ARGV) {
    ...
    } elsif ($Coloring::python eq $type) {
        open (SOURCE, "$bin_dir/pythonPreColor.pl $file|") or
            warn "Couldn't do it, dude.\n", next;
    ...
    while (<SOURCE>) {
        ...
        s|$keyStart\d+$finish|<font color=$key_color>|g;
        s|$keyEnd\d+$finish|</font>|g;
        s|$commentStart\d+$finish|<font color=$comment_color>|g;
        s|$commentEnd\d+$finish|</font>|g;
        s/ /\&nbsp\;/g;
        s/<([^>]*)&nbsp\;([^>]*)/<$1 $2/g;
