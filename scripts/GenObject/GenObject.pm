...
our $version = "GenObject.pm      v1.11 cplager 060417";
...
sub _init {
   my $self = shift;
   my $line = shift;
   # is this a 'name| 1 2 3' initialization, or a
   ...
   if ($line =~ m/\s*(\w+)\s*\|\s*(.+)/) {
      ...
      if (@parts != $elements) {
         my $num = @parts;
         warn "Particle Initialization Error:\n'$name' particles are supposed to have $elements elements.  You have provided $num.\n$line\n";
         return;
      }
