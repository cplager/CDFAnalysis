...
void dg_kin_Wuv_Wjj( TLorentzVector pu, TLorentzVector pv, 
		     ...
		     float &cosphipl, float &cosphiWH, float &ctuv, float &ctjj, float &ctWH ){

  // parent momentum is the sum of all four momenta
  TLorentzVector  ppar = pu + pv + pj1 + pj2;
  TLorentzVector  pW1 = pu + pv ;
  TLorentzVector  pW2 = pj1 + pj2;
  ...
  TVector3 crWW;
  TVector3 crjj;
  double sinuv, sinWW, sinjj;
  dg_cross2( cruv, sinuv, pust, pvst);
  dg_cross2( crWW, sinWW, pW1, pW2);
  dg_cross2( crjj, sinjj, pj1st, pj2st);


  // cosine of angle between normals of the 2 decay planes  
  if( crjj.Mag()==0.0 || cruv.Mag()==0.0) cosphipl = -1.0;
  else cosphipl = cruv.Dot(crjj) / (cruv.Mag() * crjj.Mag() );

  // cosine of angle between normals of the leptonic W decay plane and 
  // Higgs decay plane
  if( crWW.Mag()==0.0 || cruv.Mag()==0.0) cosphiWH = -1.0;
  else cosphiWH = cruv.Dot(crWW) / (cruv.Mag() * crWW.Mag() );
  ...
  //costheta between leptonic W and Higgs in Higgs rest frame
  ctWH = ppar.CosTheta();
  
  // boost jet to j1+j2 rest frame

  // which one is the "leading" jet  ???
  TLorentzVector  pjet;
  if (pj1st.E() > pj2st.E())  pjet = pj1st;
  else pjet = pj2st;

  ppar = pj1st + pj2st;
  TLorentzVector  pj1ro = dgieuler(ppar, pjet);
  gamma = ppar.Gamma();
  beta = ppar.Beta();
